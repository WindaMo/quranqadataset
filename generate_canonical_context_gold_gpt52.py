...
FILTERED_LOG = "filtered_verses.json"
...
def build_messages(surah, ayat, text, theme_paths):
    ...
    selected_path = theme_paths[0] if theme_paths else []
    theme_string = " → ".join(selected_path)
...
def generate_gold(entry):
    ...
    try:
        ...
        parsed["surah"] = surah
        parsed["ayat"] = ayat
        parsed["theme_path_used"] = theme_paths[0] if theme_paths else []
